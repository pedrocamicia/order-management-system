...
async def iniciar_pedido(user : User = Depends(get_current_user)):
    pedido = service.iniciar_pedido(user)
...
async def modificar_items_pedido(data : ModificarItemsPedidoRequest, pedido_id : int, user : User= Depends(get_current_user)):
    ...
    pedido = service.modificar_items_pedido(pedido_id, data.producto_id, data.cantidad, user)
...
def get_pedido(pedido_id : int, user = Depends(get_current_user)):
    ...
    pedido = service.get_pedido(pedido_id, user)
...
def confirmar_pedido(pedido_id : int, user = Depends(get_current_user)):
    ...
    pedido = service.confirmar_pedido(pedido_id, user)
...
def eliminar_item(pedido_id : int, producto_id : int, user = Depends(get_current_user)):
    service.eliminar_item(pedido_id, producto_id, user)
    return